...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f p = P $ \i -> case parse p i of
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = character >>= lift3 if' p valueParser unexpectedCharParser
            where if' c a b = if c then a else b
...
natural ::
  Parser Int
natural = (?? 0) . read <$> list1 (satisfy isDigit)
