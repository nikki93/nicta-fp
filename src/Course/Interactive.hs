...
reverseInteractive ::
  IO ()
reverseInteractive =
  putStr "input filename: " >- getLine >>= \input ->
  putStr "output filename: " >- getLine >>= \output ->
...
encodeInteractive ::
  IO ()
encodeInteractive =
  putStrLn "enter a string to url encode: " >- getLine >>= putStrLn . flatMap encode
  where encode ' ' = listh "%20"
        encode '\t' = listh "%09"
        encode '"' = listh "%22"
        encode a = a :. Nil
