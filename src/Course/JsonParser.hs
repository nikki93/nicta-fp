...
jsonString ::
  Parser Chars
jsonString = between (is '"') (charTok '"')
             (list ((is '\\' *> hexu)
                    ||| (fromSpecialCharacter . (?? NewLine) . toSpecialCharacter
                         <$> (is '\\' *> oneof "bfnrtv'\"\\"))
                    ||| (noneof "\\\"")))
...
jsonNumber ::
  Parser Rational
jsonNumber = P $ (?? ErrorResult Failed) . (uncurry (flip Result) <$>) . readFloats
...
jsonObject = betweenSepbyComma '{' '}'
               ((,) <$> jsonString <*> spaces *> charTok ':' *> jsonValue)
