...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l a r) = case break p l of
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l a r) = case break p r of
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil a r) = let a' :. l' = reverse (a :. r) in ListZipper l' a' Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l a Nil) = let a' :. r' = reverse (a :. l) in ListZipper Nil a' r'
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n z@(ListZipper l a r)
  | n == 0 = IsZ z
  | n < 0 = moveRightN (-n) z
  | n > length l = IsNotZ
  | otherwise = let a' :. r' = reverse $ take n l
                in IsZ $ ListZipper (drop n l) a' (r' ++ a :. r)
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n z@(ListZipper l a r)
  | n == 0 = IsZ z
  | n < 0 = moveLeftN (-n) z
  | n > length r = IsNotZ
  | otherwise = let a' :. l' = reverse $ take n r
                in IsZ $ ListZipper (l' ++ a:. l) a' (drop n r)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l a r) = let a' :. l' = reverse (a :. r) in ListZipper (l' ++ l) a' Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l a r) = let a' :. r' = reverse (a :. l) in ListZipper Nil a' (r' ++ r)
...
instance Applicative ListZipper where
  pure = lift3 ListZipper repeat id repeat
...
instance Extend ListZipper where
  (<<=) f = lift3 ListZipper (u moveLeft) f (u moveRight)
            where u m = unfoldr ((lift2 (,) f id <$>) . toOptional . m)
